...
def hint_block(network, para, hint, temb, context):
    ...
    hint_in = network.add_convolution(hint_in, 16, (3, 3), format(para['input_hint_block.2.weight']), format(para['input_hint_block.2.bias']))
    hint_in.padding = (1, 1)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 32, (3, 3), format(para['input_hint_block.4.weight']), format(para['input_hint_block.4.bias']))
    hint_in.padding = (1, 1)
    hint_in.stride = (2, 2)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 32, (3, 3), format(para['input_hint_block.6.weight']), format(para['input_hint_block.6.bias']))
    hint_in.padding = (1, 1)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 96, (3, 3), format(para['input_hint_block.8.weight']), format(para['input_hint_block.8.bias']))
    hint_in.padding = (1, 1)
    hint_in.stride = (2, 2)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 96, (3, 3), format(para['input_hint_block.10.weight']), format(para['input_hint_block.10.bias']))
    hint_in.padding = (1, 1)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 256, (3, 3), format(para['input_hint_block.12.weight']), format(para['input_hint_block.12.bias']))
    hint_in.padding = (1, 1)
    hint_in.stride = (2, 2)
    hint_in = silu(network, hint_in)
    hint_in = network.add_convolution(hint_in, 320, (3, 3), format(para['input_hint_block.14.weight']), format(para['input_hint_block.14.bias']))
    hint_in.padding = (1, 1)
    return hint_in
